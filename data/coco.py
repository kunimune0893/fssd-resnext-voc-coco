...
class COCOAnnotationTransform(object):
    ...
    def __call__(self, target, width, height):
        ...
        for obj in target:
            if 'bbox' in obj:
                bbox = copy.deepcopy(obj['bbox'])  # EDITED BY EIGO TO USE DEEPCOPY TO PREVENT BREAKING ANNO
                bbox[2] += bbox[0]
                bbox[3] += bbox[1]
...
class COCODetection(data.Dataset):
    ...
    def __init__(self, root, image_set='trainval35k', transform=None,
        ...
        self.image_indexes = self.coco.getImgIds()
    ...
    def pull_item(self, index):
        ...
        if self.transform is not None:
            if len(target) != 0:
                ...
                img, _, _ = self.transform(img, None, None) # Eigo
                target.append([0.5,0.5,1.,1.,-1.]) # Eigo dummy background object at center
                target = np.array(target)
            # to rgb
            img = img[:, :, (2, 1, 0)]
    ...
    def _coco_results_one_category(self, boxes, cat_id):
        ...
        for im_ind, index in enumerate(self.image_indexes):
            # print(type(boxes[im_ind]), boxes[im_ind])
            if boxes[im_ind] == []:
                continue
    ...
    def _write_coco_results_file(self, all_boxes, res_file):
        ...
        for cls_ind, cls in enumerate(self._classes):
            if cls == '__background__':
                continue
            print('Collecting {} results ({:d}/{:d})'.format(cls, cls_ind, 80 ))
